document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- PANEL ADMINISTRATORA (WIZUALNY EDYTOR) ---
    const isAdmin = sessionStorage.getItem('ksite_admin') === 'true';

    if (isAdmin) {
        const toolbar = document.getElementById('admin-toolbar');
        if (toolbar) toolbar.classList.remove('hidden');

        // Ustaw edytowalne elementy na stronie
        const editableElements = document.querySelectorAll('h1, h2, h3, h4, p, span.text-indigo-400, span.text-violet-400, .text-sm.font-bold, .text-lg');
        
        editableElements.forEach(el => {
            // Wyklucz z edycji elementy techniczne (toolbar, formularze, ikonki, buttony)
            if (!toolbar.contains(el) && !el.closest('form') && !el.closest('button') && !el.querySelector('i')) {
                el.setAttribute('contenteditable', 'true');
                el.classList.add('hover:outline', 'hover:outline-2', 'hover:outline-violet-500', 'hover:bg-violet-500/10', 'transition-all', 'cursor-text', 'rounded');
            }
        });

        // Zabezpieczenie przed przeładowaniem strony przy Enterze na tytułach
        editableElements.forEach(el => {
            el.addEventListener('keydown', function(e) {
                if(e.key === 'Enter' && !el.matches('p')) {
                    e.preventDefault(); // Enter zablokowany w h1, h2, h3 itp.
                }
            });
        });

        // Funkcja pobierania zaktualizowanego pliku
        window.saveAndDownload = function() {
            const htmlCopy = document.documentElement.cloneNode(true);
            
            // Ukryj pasek przed zapisaniem
            const toolbarClone = htmlCopy.querySelector('#admin-toolbar');
            if (toolbarClone) toolbarClone.classList.add('hidden');
            
            // Usuń tryb edycji z tekstów w zapisanej kopii
            const editables = htmlCopy.querySelectorAll('[contenteditable="true"]');
            editables.forEach(el => {
                el.removeAttribute('contenteditable');
                el.classList.remove('hover:outline', 'hover:outline-2', 'hover:outline-violet-500', 'hover:bg-violet-500/10', 'transition-all', 'cursor-text', 'rounded');
            });
            ...
            alert('Plik pobrany! Zastąp stary plik index.html tym nowym (np. na GitHubie lub u siebie na dysku).');
        };

        window.logoutAdmin = function() {
            sessionStorage.removeItem('ksite_admin');
            window.location.reload();
        };
    }

    // Dodatkowe wsparcie dla #admin prosto z linku (wygoda dla Czystego HTMLa)
    ...
    // Mobilne menu - Niezawodne przełączanie
